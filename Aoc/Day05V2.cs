...
public class Day05V2 : ISimpleSolution
{
    ...
    public void Solve(ReadOnlySpan<char> input)
    {
        ...
        {
            var left = int.Parse(input.Slice(inputIdx, 2));// input[inputIdx] - '0')) * 10 + (input[inputIdx + 1] - '0');
            var right = int.Parse(input.Slice(inputIdx + 3, 2));//(input[inputIdx + 3] - '0') * 10 + (input[inputIdx + 4] - '0');
            inputIdx += 6;
